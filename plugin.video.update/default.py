...
AddonTitle='Iptvbrasil UPDATE'; wizardUrl='https://copy.com/'; 
SiteDomain='iptvbr.org'; TeamName='IPTV BRASIL GROUP'; 
addon=main.addon; net=main.net; settings=main.settings; 
SkinBackGroundImg=os.path.join('special://','home','media','SKINDEFAULT.png')
...
LinksUrl=wizardUrl+'VeHiN6YJxbEmOZN9?download=1'
...
def HELPWIZARD(name,url,description,filetype):
    ...
    if filetype=='splash':
        ...
        return
    if confirm.yesno(TeamName,"[B]Gostaria de configurar seu xbmc/kodi com nossa selecao de add-ons?[/B]"," "):
        dp=xbmcgui.DialogProgress(); dp.create(AddonTitle,"[B] Efetuando Download [/B] ",'','[B] Aguarde [/B]...')
        ...
        #time.sleep(2)
        xbmc.sleep(4000)
        dp.update(0,"","[B][COLOR gold] Extraindo Zip por favor aguarde...[/COLOR][/B]")
        ...
        if (filetype=='main') and (settings.getSetting('homescreen-shortcuts')=='true'):
            link=main.OPEN_URL(wizardUrl+'ZuVz0heBtnltrVLG?download=1')
        ...
        dialog=xbmcgui.Dialog(); dialog.ok("Sucesso!","Instalacao Completa!","   [COLOR gold]Visite nosso site %s[/COLOR]"%SiteDomain)
        ##
#==========
